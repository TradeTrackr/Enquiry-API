...
def unhandled_exception(e):
    ...
    return Response(response=json.dumps({"error_message":"unhandled_exception", "error_code":"xxx"}), status=500)
...
def database_exception(e):
    return Response(response=json.dumps({"error_message": e.message, "error_code": e.http_code}), status=e.http_code)


def register_exception_handlers(app):
    app.register_error_handler(ApplicationError, application_error)
    app.register_error_handler(DatabaseException, database_exception)
